# -*- coding: utf-8 -*-
"""agentes_inteligentes_break_walls.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1EzcB4ZzHNV4z9AdEQH5XDt1Xsp8mOhDG
"""
...
class TacticalAgent(Agent):
    def __init__(self, unique_id, model):
        ...
        self.current_path = []  # Path from Dijkstra
        self.current_goal = None  # Current target position
        self.current_task = None  # Current task type for debugging

    def step(self):
        self.action_points = 4

        while self.action_points > 0:
            # Execute actions based on strict hierarchy
            action_taken = self._execute_hierarchy()
            if not action_taken:
                break

    def _execute_hierarchy(self):
        """Execute actions based on strict priority hierarchy"""

        # 1. EVACUAR REHÉN CARGADO (Highest Priority)
        if self.carrying_hostage:
            return self._evacuate_hostage()

        # 2. RESCATAR REHÉN (Second Priority)
        hostage_action = self._rescue_hostage()
        if hostage_action:
            return hostage_action

        # 3. CONTENER DISTURBIO (Third Priority)
        disturbance_action = self._contain_disturbance()
        if disturbance_action:
            return disturbance_action

        # 4. INVESTIGAR FALSA ALARMA (Fourth Priority)
        alarm_action = self._investigate_alarm()
        if alarm_action:
            return alarm_action

        # 5. EXPLORAR MAPA (Lowest Priority)
        return self._explore_map()

    def _evacuate_hostage(self):
        """Evacuate carried hostage to nearest entry point"""
        if not self.carrying_hostage:
            return False

        # If already at entry point, drop off hostage
        if self.pos in self.model.entry_points:
            if self.action_points >= 1:
                ...
                self.current_task = "dropoff_complete"
                return True
            return False

        # Find path to nearest entry point
        if not self.current_path or self.current_goal not in self.model.entry_points:
            nearest_entry = self._find_nearest_entry_point()
            if nearest_entry:
                self.current_path = self.model.dijkstra_path(self.pos, nearest_entry)
                self.current_goal = nearest_entry
                self.current_task = "evacuating_hostage"

        # Move along path
        return self._move_along_path()

    def _rescue_hostage(self):
        """Rescue nearby hostage"""
        if self.carrying_hostage:
            return False

        # Check if hostage in current cell
        current_contents = self.model.get_contents_at(self.pos)
        hostage_found = next((c for c in current_contents if isinstance(c, Hostage)), None)
        ...
        if hostage_found and self.action_points >= 2:
            ...
            self.model.remove_entity(hostage_found, self.pos)
            self.action_points -= 2
            self.current_path = []
            self.current_goal = None
            self.current_task = "rescued_hostage"
            return True

        # Look for nearby hostages
        if not self.current_path or not self._is_goal_hostage():
            hostage_pos = self._find_nearest_hostage()
            if hostage_pos:
                path = self.model.dijkstra_path(self.pos, hostage_pos)
                if path and len(path) > 1:  # Ensure path exists and is not just current position
                    self.current_path = path
                    self.current_goal = hostage_pos
                    self.current_task = "moving_to_hostage"
                    return self._move_along_path()
        else:
            # Continue moving to current hostage goal
            return self._move_along_path()
    ...
    def _contain_disturbance(self):
        ...
        # Check if disturbance in current cell
        current_contents = self.model.get_contents_at(self.pos)
        disturbance_found = next((c for c in current_contents if isinstance(c, Disturbance)), None)

        if disturbance_found:
            required_points = 1 if disturbance_found.severity == 'mild' else (2 if disturbance_found.severity == 'active' else 999)
            if self.action_points >= required_points and required_points < 999:
                self.model.remove_entity(disturbance_found, self.pos)
                if hasattr(self.model, "logger"):
                    self.model.logger.riot_contained(
                        self.pos[1] + 1, self.pos[0] + 1, t=self.model.turn_counter + 1
                    )
                self.action_points -= required_points
                self.current_path = []
                self.current_goal = None
                self.current_task = "contained_disturbance"
                return True

        # Look for nearby disturbances
        if not self.current_path or not self._is_goal_disturbance():
            disturbance_pos = self._find_nearest_containable_disturbance()
            if disturbance_pos:
                path = self.model.dijkstra_path(self.pos, disturbance_pos)
                if path and len(path) > 1:
                    self.current_path = path
                    self.current_goal = disturbance_pos
                    self.current_task = "moving_to_disturbance"
                    return self._move_along_path()
        else:
            return self._move_along_path()
    ...
    def _investigate_alarm(self):
        ...
        # Check if alarm in current cell
        current_contents = self.model.get_contents_at(self.pos)
        alarm_found = next((c for c in current_contents if isinstance(c, FalseAlarm)), None)

        if alarm_found and self.action_points >= 1:
            self.model.reveal_if_needed(self.pos)
            self.model.false_alarms_investigated += 1
            self.model.remove_entity(alarm_found, self.pos)
            self.action_points -= 1
            self.current_path = []
            self.current_goal = None
            self.current_task = "investigated_alarm"
            return True

        # Look for nearby alarms
        if not self.current_path or not self._is_goal_alarm():
            alarm_pos = self._find_nearest_alarm()
            if alarm_pos:
                path = self.model.dijkstra_path(self.pos, alarm_pos)
                if path and len(path) > 1:
                    self.current_path = path
                    self.current_goal = alarm_pos
                    self.current_task = "moving_to_alarm"
                    return self._move_along_path()
        else:
            return self._move_along_path()
    ...
    def _explore_map(self):
        ...
        # Look for unexplored areas or handle other low-priority actions
        if not self.current_path or not self._is_exploring():
            unexplored_pos = self._find_unexplored_area()
            if unexplored_pos:
                path = self.model.dijkstra_path(self.pos, unexplored_pos)
                if path and len(path) > 1:
                    self.current_path = path
                    self.current_goal = unexplored_pos
                    self.current_task = "exploring"
                    return self._move_along_path()
        else:
            return self._move_along_path()

        # If no exploration target, try other actions like opening gates
        return self._handle_miscellaneous_actions()

    def _move_along_path(self):
        """Move along the current Dijkstra path"""
        if not self.current_path or len(self.current_path) <= 1:
            return False

        next_pos = self.current_path[1]  # Next position in path (current path[0] should be current position)
        ...
        next_contents = self.model.get_contents_at(next_pos)
        cost = 1
        ...
        if self.action_points >= cost:
            ...
            # Reveal POI if needed
            revealed_now = self.model.reveal_if_needed(self.pos)
            if revealed_now:
                self.action_points = 0  # End turn after revelation

            return True

        return False

    def _handle_miscellaneous_actions(self):
        """Handle gates, walls, and other miscellaneous actions"""
        current_contents = self.model.get_contents_at(self.pos)

        # Handle gates
        gate_found = next((c for c in current_contents if isinstance(c, Gate)), None)
        if gate_found and self.action_points >= 1:
            gate_found.is_open = not gate_found.is_open
            self.action_points -= 1
            return True

        # Break walls if necessary (as last resort)
        if self.action_points >= 2:
            neighbors = self.model.grid.get_neighborhood(self.pos, moore=False, include_center=False)
            for neighbor_pos in neighbors:
                if self.model.has_wall_between(self.pos, neighbor_pos):
                    self.model.break_wall_between(self.pos, neighbor_pos)
    ...
    def _find_nearest_entry_point(self):
        """Find nearest entry point using Dijkstra distance"""
        min_distance = float('inf')
        nearest_entry = None

        for entry in self.model.entry_points:
            distance = self.model.dijkstra_distance(self.pos, entry)
    ...
    def _find_nearest_hostage(self):
        """Find nearest hostage position"""
        min_distance = float('inf')
        nearest_pos = None

        for pos, contents in self.model.cell_contents.items():
            if any(isinstance(c, Hostage) for c in contents):
                distance = self.model.dijkstra_distance(self.pos, pos)
                if distance < min_distance:
                    min_distance = distance
                    nearest_pos = pos
    ...
    def _find_nearest_containable_disturbance(self):
        """Find nearest containable disturbance (mild or active only)"""
        min_distance = float('inf')
        nearest_pos = None

        for pos, contents in self.model.cell_contents.items():
            for content in contents:
                if isinstance(content, Disturbance) and content.severity in ['mild', 'active']:
                    distance = self.model.dijkstra_distance(self.pos, pos)
                    if distance < min_distance:
                        min_distance = distance
                        nearest_pos = pos
    ...
    def _find_unexplored_area(self):
        """Find an unexplored area to investigate"""
        # Simple exploration: find empty cells that might contain hidden POIs
        potential_targets = []

        for x in range(self.model.grid.width):
            for y in range(self.model.grid.height):
                pos = (x, y)
                contents = self.model.get_contents_at(pos)

                # Target empty cells or cells with only gates
                if not contents or all(isinstance(c, Gate) for c in contents):
                    if pos not in self.model.revealed_pois:
        ...
        if potential_targets:
            ...
            for target in potential_targets:
                distance = self.model.dijkstra_distance(self.pos, target)
                if distance < min_distance:
                    min_distance = distance
                    best_target = target
    ...
    # Helper methods for checking current goals
    def _is_goal_hostage(self):
        """Check if current goal contains a hostage"""
        if not self.current_goal:
            return False
        contents = self.model.get_contents_at(self.current_goal)
        return any(isinstance(c, Hostage) for c in contents)

    def _is_goal_disturbance(self):
        """Check if current goal contains a containable disturbance"""
        if not self.current_goal:
            return False
        contents = self.model.get_contents_at(self.current_goal)
        for content in contents:
    ...
    def _is_goal_alarm(self):
        """Check if current goal contains a false alarm"""
        if not self.current_goal:
            return False
        contents = self.model.get_contents_at(self.current_goal)
        return any(isinstance(c, FalseAlarm) for c in contents)

    def _is_exploring(self):
        """Check if currently exploring"""
        return self.current_task == "exploring"

# ------------------------------------------------------------
#                     ENHANCED RESCUE MODEL
# ------------------------------------------------------------
class RescueModel(Model):
    def __init__(self, config_path="config.json"):
        ...
        # Pathfinding cache for optimization
        self._distance_cache = {}

        # Agentes iniciales (6) en entradas
        for _ in range(6):
        ...
        self.datacollector = DataCollector(
            model_reporters={"Grid": lambda m: np.array(self.get_grid_board())}
        )
    ...
    def dijkstra_path(self, start, goal):
        ...
        while pq:
            ...
            for neighbor in neighbors:
                ...
                if not self.can_move_to(current_pos, neighbor):
                    continue

                # Calculate movement cost
                neighbor_contents = self.get_contents_at(neighbor)
                movement_cost = 1
    ...
    def dijkstra_distance(self, start, goal):
        ...
        while pq:
            ...
            if current_pos == goal:
                self._distance_cache[cache_key] = current_dist
                return current_dist
    ...
    def clear_pathfinding_cache(self):
        """Clear pathfinding cache when environment changes"""
    ...
    def break_wall_between(self, pos1, pos2):
        ...
        # Clear pathfinding cache when walls change
